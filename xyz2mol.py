...
##########################
'''
Created on Mon Feb 06 18:25:04 2023

@author: chingchinglam

code to apply the above function for CONFPASS
...
def list_duplicates(seq):
    ...
    return [[len(locs),key,sorted(locs)] for key,locs in tally.items() if len(locs)>0]



def mol_separations(sdf_name,delG_csv_name,sdf_cs):


    suppl = Chem.SDMolSupplier(sdf_name, removeHs=False)
    mol_ls=[mol for mol in suppl]
    ...
    for mol in mol_ls:
        ...
        new_mols = xyz2mol(atoms, coordinates, charge=charge)[0]
    
        mol_ls2.append(new_mols)


    inchi_mol_ls=[Chem.inchi.MolToInchi(mol) for mol in mol_ls2]

    result0 = sorted(list_duplicates(inchi_mol_ls), key = lambda x: int(x[0]), reverse=True)
    
    cs_suppl=  Chem.SDMolSupplier(sdf_cs, removeHs=False)
    origin_stru_inchi = Chem.inchi.MolToInchi([mol for mol in cs_suppl][0])
    
    origin_idx=0
    
    for idx in range(0,len(result0)):
        if origin_stru_inchi in result0[idx]: 
            origin_idx=idx
            
    result = [result0[origin_idx]]
    ...
    ### to separate sdf files 
    sdf_ls=[]
    for ls in result:
        sub_sdf_txt=''
        for i in ls[-1]:
            sub_sdf_txt+=Chem.MolToMolBlock(mol_ls2[i])+'\n$$$$\n'
        sdf_ls.append(sub_sdf_txt)


    
    name_ls=[sdf_name]
    name_delG_ls=[delG_csv_name]
    for idx in range(1,len(result)):
        name=sdf_name[:-4]+'_'+str(idx)+'.sdf'
        name_ls.append(name)
    
        name2=delG_csv_name[:-4]+'_'+str(idx)+'.csv'
        name_delG_ls.append(name2)
    
    for c,d in zip(name_ls,sdf_ls):
        ## write the file
    ...
    ### to separate csv file (energy info)
    
    delG_result=pd.read_csv(delG_csv_name)
    
    df_ls=[]
    for ls in result:
        new_df=delG_result.loc[delG_result['idx'].isin(ls[-1])].copy()
        new_df['idx']=list(range(0,len(new_df)))
        
        min_G=min(new_df['G_hatree'])
        min_H=min(new_df['H_hatree'])

        new_df['delH_kcal']=(new_df['H_hatree'] - min_H)*627.509
        new_df['delG_kcal']=(new_df['G_hatree'] - min_G)*627.509
    
        new_df['delG_kJ']=(new_df['G_hatree'] - min_G)*2625.5
    
        df_ls.append(new_df)

    for df,name in zip(df_ls,name_delG_ls):
